...
let rename_variable_identifier x y id =
  { id with name = String.substr_replace_all id.name ~pattern:x ~with_:y }

let rec rename_variable_expr x y { expr; emeta } =
  match expr with
  | Variable id ->
      { expr = Variable (rename_variable_identifier x y id); emeta }
  | _ -> { expr = map_expression (rename_variable_expr x y) expr; emeta }
...
let rename_variable_lval x y { lval; lmeta } =
  let is_multiindex = function Single _ -> false | _ -> true in
  let rec flatten_multi = function
    | LVariable id -> (LVariable (rename_variable_identifier x y id), None)
    | LTupleProjection ({ lval; lmeta }, i) -> (
      match flatten_multi lval with
      | lval, inner -> (LTupleProjection ({ lval; lmeta }, i), inner))
    | LIndexed ({ lval; lmeta }, idcs) -> (
        let outer = List.map idcs ~f:(map_index (rename_variable_expr x y)) in
        ...
        | lval, Some (inner, _) -> (lval, Some (inner @ outer, lmeta)))
  in
  let lval =
    match flatten_multi lval with
    | lval, None -> lval
    | lval, Some (idcs, lmeta) -> LIndexed ({ lval; lmeta }, idcs)
  in
  { lval; lmeta }
...
let rec rename_variable_stmt x y ({ stmt; smeta } : untyped_statement) :
    untyped_statement =
  let stmt =
    match stmt with
    | VarDecl { decl_type; transformation; variables; is_global } ->
        VarDecl
          {
            decl_type =
              Middle.SizedType.map (rename_variable_expr x y) decl_type;
            transformation =
              Middle.Transformation.map (rename_variable_expr x y)
                transformation;
    ...
    | _ ->
        map_statement (rename_variable_expr x y) (rename_variable_stmt x y)
          (rename_variable_lval x y) stmt
  in
  { stmt; smeta }

let rename_variable x y ast = ast |> map_program (rename_variable_stmt x y)
