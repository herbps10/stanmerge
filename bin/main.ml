...
let get_ast_from_config (model_file, rules) =
  ...
  List.fold rules ~init:ast ~f:(fun acc (x, y) -> Transform.Rename.rename_variable (Str.regexp x) y acc)
...
let parse_rules (name, values) = 
  let parsed_values = List.map ~f:parse_rule (Yojson.Basic.Util.to_assoc values) in
  (name, parsed_values)
...
let main () = 
  ...
  if (String.compare !use_config "") <> 0 then
    ...
    print_endline
      (Merge.merge_asts asts)
  else 
    let asts = List.map !model_files ~f:get_ast in
    let asts_renamed = List.map asts ~f:(Transform.Rename.rename_variable
    (Str.regexp "var") "alpha") in 
    print_endline 
      (Merge.merge_asts asts_renamed)
